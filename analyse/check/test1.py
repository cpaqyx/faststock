...
class MyStrategy(strategy.BacktestingStrategy):
    ...
    def onBars(self, bars):
        ...
        self.info("可用现金%.2f 股价%.2f 持股数量%d 市值1:%.2f 市值2:%.2f 计算市值:%.2f 交易成本%.2f" % (
        brk.getCash(), price, brk.getShares(self.__instrument), brk.getEquity(), self.getResult(),
        (brk.getCash() + brk.getShares(self.__instrument) * price), self.__cost))
    # x = input("按任意键继续")
...
def run_strategy(cash):
    ...
    # instruments = ["600036"]
    # feeds = stock_line_day_dao().get_stock_line_day("600036.SH", "20200601", "20221230")
    ...
    myStrategy.run()
    plter.savePlot(dpi=500, filename="testdata.png")
